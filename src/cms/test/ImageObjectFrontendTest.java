...
public final class ImageObjectFrontendTest {
    ...
    public static void run(TestRunner.TestContext ctx) {
        ...
        ctx.test("stored javascript: and data: URLs render as inert text, never as links", () -> {
            String jsId = Helpers.seedWith(ENTITY, Map.of("contentUrl", "javascript:alert(1)"));
            ...
            String dataId = Helpers.seedWith(ENTITY, Map.of("contentUrl", "data:text/html,x"));
        ...
        ctx.test("stored http(s) URL renders as a clickable link", () -> {
            String id = Helpers.seedWith(ENTITY, Map.of("contentUrl", "https://example.com/profile"));
            Helpers.Response r = Helpers.frontendGet(BASE + "/" + id);
            Assert.equal(200, r.status);
